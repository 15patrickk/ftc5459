...
#include "hitechnic-irseeker-v2.h" // handles the IR sensors
...
void driveFor(float rotations, bool direction) {
	...
	nMotorEncoderTarget[driveL] = nMotorEncoderTarget[driveR] = rotations * 1440;
	if (direction) {
		motor[driveL] = motor[driveR] = 30;
		while(nMotorRunState[driveL] != runStateIdle || nMotorRunState[driveR] != runStateIdle) { }
		motor[driveL] = motor[driveR] = 0;
	}
	else {
		nMotorEncoderTarget[driveL] = nMotorEncoderTarget[driveR] =  nMotorEncoderTarget[driveL] *= -1;
		motor[driveL] = motor[driveR] = -30;
		while(nMotorRunState[driveL] != runStateIdle || nMotorRunState[driveR] != runStateIdle) { }
		motor[driveL] = motor[driveR] = 0;
	}
...
void ramp() {
	// in theory: while the robot is more than 5cm away from the wall, drive forward
	while(SensorValue(frontSonar) > 5) {
		motor[driveL] = motor[driveR] = 40;
	}
	motor[driveL] = motor[driveR] = 0; // STOP once we get near the wall

	// in theory: turn 90 degrees right
	while(nMotorEncoder[driveL] < 1440) {
		motor[driveL] = 40;
		motor[driveR] = -40;
	}
	nMotorEncoder[driveL] = 0; // reset the encoder
...
task main() {
	...
	if (checkBeacon()) { // found at 1st crate
		...
		driveFor(0.71, true); // drive to 2nd crate
		if (checkBeacon()) { // found at 2nd crate
			...
			driveFor(1.8, true); // longer to move past the pendulum's midsection
